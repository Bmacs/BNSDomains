...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Buds Naming Service", "BNS") {
  ...
  function getAllNames() public view returns (string[] memory) {
    ...
    for (uint i = 0; i < _tokenIds.current(); i++) {
      allNames[i] = names[i];
      console.log("Name for the token %d is %s", i, allNames[i]);
    }
    return allNames;
  }
      ...
      string memory json = Base64.encode (
        ...
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Buds naming service", "image": "data:image/svg+xml;base64,',
